#!/usr/bin/env python3
"""Run the locked Phase 11.6 Tavily citation-contract isolation calibration.

The already-observed Phase 10 cases are reused transparently. Each case makes
one Tavily request. The resulting selected and projected packet is then shared
byte-for-byte by a legacy-prompt arm and a strict-citation arm. Questions,
...
BENCHMARK_NAME = "evidencemesh-phase11-6-tavily-citation-isolation-v1"
PROMPT_VERSION = "evidence-answer-phase11-6-v1"
LOCKED_PROTOCOL_SHA256 = "d72aa29b989d74cff09ca211e291b5d2d58d86d684fe6bca5f75a0dac429c4e8"
...
MINIMUM_COMPLETIONS_PER_MODEL_ARM = 11
MINIMUM_PROMPT_ANSWER_COVERAGE = 10
MINIMUM_CITATION_PRESENCE = 0.90
MINIMUM_CITATION_ID_VALIDITY = 1.0
MINIMUM_CITATION_SUPPORT = 0.70
...
def build_prompt(
    ...
    *,
    ...
    if bundle.arm != BASELINE_ARM:
        raise ValueError(f"unknown Phase 11.6 generation arm: {bundle.arm}")
    user_prompt = (
        f"Question:\n{row.question}\n\n"
        f"Evidence blocks:\n{rendered}\n\n"
...
async def retrieve_all(
    ...
    *,
    ...
    settings = Settings(
        ...
        cache_path=cache_root / "phase11-6.sqlite3",
...
def build_decision(
    ...
    *,
    ...
    gates = {
        ...
        "strict_citation_presence_at_least_90_percent": {
        ...
        "strict_citation_support_at_least_70_percent": {
            "passed": (
    ...
    return {
        "gates": gates,
        "phase11_6_candidate_passed": candidate_passed,
        ...
        "reason": (
            "Phase 11.6 is a disclosed causal calibration on reused cases. A pass "
            "permits a separately committed Tavily-only quality profile and can "
            "unblock, but never execute, a separately authorized Phase 12 run."
        ),
    }
...
def validate_arguments(arguments: argparse.Namespace) -> None:
    if tuple(arguments.models) != MODELS:
        raise ValueError(f"Phase 11.6 requires the exact locked model order: {MODELS}")
    locked_values = {
    ...
    if changed:
        raise ValueError("Phase 11.6 locked arguments changed: " + ", ".join(changed))
    if arguments.gemini_api_key_env != "GEMINI_API_KEY":
        raise ValueError("Phase 11.6 requires GEMINI_API_KEY as the runtime environment name")
    if arguments.tavily_api_key_env != "TAVILY_API_KEY":
        raise ValueError("Phase 11.6 requires TAVILY_API_KEY as the runtime environment name")
...
async def run(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    if sha256_bytes(protocol_bytes) != LOCKED_PROTOCOL_SHA256:
        raise ValueError("Phase 11.6 protocol checksum does not match the frozen lock")
    ...
    return {
        ...
        "task_type": "Tavily-only citation-contract causal calibration",
        "suite": {
            ...
            "purpose": (
                "Phase 10 cases deliberately reused for causal prompt calibration; "
        ...
        "warnings": [
            ...
            (
                "The suite was already observed in Phases 10 and 11.5 and is "
                "calibration-only. A pass cannot establish generalization or superiority."
            ),
            (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=root / "docs/benchmark-protocol-v14.md",
    )
    ...
    parser.add_argument(
        "--cache-root",
        type=Path,
        default=Path(tempfile.gettempdir()) / "evidencemesh-phase11-6",
    )
